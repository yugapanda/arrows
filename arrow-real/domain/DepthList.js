...
module.exports = class DepthList {
  ...
  getCluster(target, nearLimit) {
    ...
    while (true) {
      //console.log(target);
      const targetIndex = this.rangeIndexer(
        target[1].point.x,
        target[1].point.y,
        nearLimit,
        nearLimit
      ...
      if (mostNear != undefined) {
        // console.log(mostNear);
        results.push([
          mostNear.depth.point.getIndex(this.width),
          mostNear.depth
        ]);
  ...
  getMostNear(depth, depthList, results) {
    return depthList.reduce((acc, now) => {
      if (
        results.some(
          r => r[1].point.x == now.point.x && r[1].point.y == now.point.y
        ) ||
        (depth.point.x == now.point.x && depth.point.y == now.point.y)
      ) {
        return acc;
      }
      const dist =
        Math.abs(depth.point.x - now.point.x) +
        Math.abs(depth.point.y - now.point.y);
      //console.log(dist);
